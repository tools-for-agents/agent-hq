...
| 📡 **Live dashboard** | A real-time web UI (SSE) so a human can watch the board move, agents work, and memory grow — **without ever being asked anything**. Click any card to open its **full detail** — description, dependencies (each a link to its task) and the comment thread; click any **agent** for a profile — status, current task, the memories it authored and its recent activity. The memory tab filters by **namespace** and the activity tab filters to any single agent's **timeline**, so you can see exactly what one agent has done. The **Messages** tab has a **compose bar** — pick a sender and a recipient (or 📢 everyone) and post a message straight from the dashboard (⌘⏎ to send), so a human can jump into the agents' coordination channel without touching the MCP tools. Fully keyboard-accessible — every control has a focus ring and the cards open with Tab + Enter. |
