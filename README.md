...
| 💸 **Spend by model** | The ledger has always computed cost per model (and the price table) and the dashboard only ever showed cost per *agent*. **Who** spent is a fact about your team; **what it cost per token** is a fact you can act on. The Ledger now breaks spend down by model with the rate that decides things — `$0.0015/1k tok` next to `$0.0314/1k tok` — so an expensive model doing cheap work is visible as the one line item you can actually change. (Two models can spend the same and differ 20× per token; the per-agent view can't tell you that.) |
| ✓✓ **Read receipts** | The messages tab said a message was **sent**; it never said whether anyone had **seen** it. The read state has been recorded per agent all along (an agent pulling its inbox marks it read) and nothing surfaced it — so *"📢 everyone"* told you a broadcast went out, not that the company got it. Now every message shows **✓ read by 2 of 4**, with the agents who've seen it in colour and the ones still waiting greyed out. A message nobody has read is not a message that was delivered; it is a message that is still waiting. |
| ⛔ **Blocked work looks blocked** | Dependencies decided what could actually be worked — `kanban_next_task` has always refused to hand out a blocked task — but the **board never said so**: a task waiting on three others looked exactly like one you could start now. Now a blocked card is dimmed and carries **⛔ blocked by N** (hover to see what it's waiting on), and the other side of it is surfaced too: the task others are queued behind is marked **🔑 frees N**. In a company of agents that can all start something, the question worth answering on sight is *which one thing unblocks the most*. |
...
**2026-06-27 and nobody noticed for two weeks** — the entire design-system pass, every
...
```
✗ STALE — http://localhost:7700 is running code that is not in your working tree.
...
It exits non-zero, so it can gate a loop or a commit.
